import { IProducto } from '../entities/Producto';
...
export class ProductoDao extends AbstractDao<IProducto> {
  ...
  public async getProductosById(identifier: string) {
    try {
  ...
  public async updateProducto(updateCashFlow: IProducto) {
    try {
  ...
  public async getCountProductos(userId: string) {
    try {
      return await super
        .getCollection()
        .countDocuments({ userId: new ObjectId(userId) });
